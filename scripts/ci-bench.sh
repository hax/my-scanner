...
# 样本清单:校验外的基准与 swc/oxc 对照共用(单一来源 scripts/samples-files.sh)
...
zig build -Doptimize=ReleaseFast bench -- --repeats="$REPEATS" $REFRESH_ZIG --json="$OUT/zig.json" "${SAMPLES_FILES[@]}"
...
# 多位图流水线实验 crate)歧义内部自决、不接受注入,进矩阵前须过 spans 校验。
scripts/prepare-lexbench.sh
DEC=build/lexbench-decisions
mkdir -p "$DEC"
for f in "${SAMPLES_FILES[@]}"; do
...
if [ "${SKIP_DIFF:-0}" != "1" ]; then
  echo "---- oxc_bitmap spans 校验(歧义自决策的等价验证) ----"
  node tools/compare-oxc-bitmap.mjs "${SAMPLES_FILES[@]}"
fi
node scripts/run-rs-bench.mjs --repeats="$REPEATS" --regex-dir="$DEC" --json="$OUT/rs.json" $REFRESH_RS "${SAMPLES_FILES[@]}"
