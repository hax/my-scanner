...
| --- | --- | --- | --- | --- |
| T1 | `)` `]` `}` 后以 `/` 开头的正则字面量语句（`if (x) /re/.test(y)`） | `)`/`]` 后判除号；`}` 后判正则 | parser 按语法位置精确判定 | 单 lexeme 回看启发式（[scanner.zig](../src/scanner.zig) `regexAllowedAfter`）：`}` 后是块尾新语句的常见形态；`)` 后接正则语句、`}` 后接对象除法在真实代码里几乎不出现 |
