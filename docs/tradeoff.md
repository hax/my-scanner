...
| T2 | 合法标识符后裸贴非 ID_Continue、非空白的 unicode 字符（`变量☃ = 1`，本身是非法 JS） | 非空白非 ASCII 字节一律按 ID-like 相互连接（boundary v2 设计），☃ 整个被越过，token 流中消失（孤立出现的 ☃ 仍正常产 `illegal`） | tsc 产 error token | 阶段 1 无上下文，精确剔除需付出额外分类平面；「只保证合法源码」假设下的已知容错差异（[simd.zig](../src/simd.zig) boundary v2 注释） |
...
| # | 输入形态 | 现状 | 期望行为 | 备注 |
| --- | --- | --- | --- | ---|
| L1 | 模板子表达式内含带 `}` 的正则（`` `${ /}/ }` ``） | 平衡扫描被正则 pattern 里的 `}` 骗过，模板 token 提前结束、后续错位 | tsc 正确 lex 完整模板 | 精确判别需完整正则/除号语义，极罕见；根治方案是模板子表达式递归调 scanner 本体（roadmap） |
