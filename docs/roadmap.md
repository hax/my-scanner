...
- [ ] **惰性 LineIndex 口径提案**：主干口径要求行号成本计入 scanInto 计时（architecture.md），jump_vec 为此付 classifyLineBreaks 一趟（占 7-25%，语料谱系相关）；真实下游（报错/sourcemap）按需查询，零查询零成本。exp 分支实测该口径下 real 全面反超 yuku-main。待 hax 裁决：维持计时口径，还是允许惰性索引（Result 接口不变，line_count/lines 首次访问时物化）
