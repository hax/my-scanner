...
- [ ] 与 esbuild / swc / oxc 的 scanner 吞吐对比：**swc/oxc 决策注入已打通**（2026-09-16，见 [architecture.md](architecture.md) 的「swc/oxc 决策注入」一节）——swc 零 patch（公开 `input::Tokens` trait 的 `set_next_regexp` / `rescan_template_token`），oxc 仅 2 行可见性 patch；全 10 语料 0 错误扫至 EOF，real 语料 my-scanner 全面 ≥ oxc-driven。待办：bench.sh/ci-bench.sh 调度链接入矩阵；esbuild 未开工
